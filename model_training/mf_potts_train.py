...
data_minus = Data(args.infile2,40,3)
...
plus_energies_train_plus = model_plus.energies(data_plus.data[data_plus.ix_train])
minus_energies_train_plus = model_minus.energies(data_plus.data[data_plus.ix_train])
plt.plot(plus_energies_train_plus,minus_energies_train_plus)
plt.show()
asdf
P_train_plus = np.exp(- plus_energies_train_plus + minus_energies_train_plus)

plus_energies_train_minus = model_plus.energies(data_minus.data[data_minus.ix_train])
minus_energies_train_minus = model_minus.energies(data_minus.data[data_minus.ix_train])
...
plus_energies_test_plus = model_plus.energies(data_plus.data[data_plus.ix_test])
minus_energies_test_plus = model_minus.energies(data_plus.data[data_plus.ix_test])
P_test_plus = np.exp(- plus_energies_test_plus + minus_energies_test_plus)

plus_energies_test_minus = model_plus.energies(data_minus.data[data_minus.ix_test])
minus_energies_test_minus = model_minus.energies(data_minus.data[data_minus.ix_test])
...
test_prob = np.concatenate((P_test_plus,1-P_test_plus))
test_auc = metrics.roc_auc_score(test_true,test_prob)
print(f'Test AUC {test_auc}')

test_fpr,test_tpr,thresholds = metrics.roc_curve(test_true,test_prob)
