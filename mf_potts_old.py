...
class mf_potts():
	...
	def train(self):
		...
		training_samples = [self.data.data[i][:,1:].astype('int32') for i in self.data._get_idx('train')]
	...
	def test(self,control):
		# Select validation data
		val_samples = np.stack([self.data.data[i][:,1:].astype('int32') for i in self.data._get_idx('val')])
		cont_samples = np.stack([control.data[i][:,1:].astype('int32') for i in control._get_idx('val')])
		...
		# Field energies for validation samples
		control_energies = np.tensordot(cont_samples,self.h,axes=2)
		# Coupling energies for validation samples
		control_energies -= np.einsum('ijk,jklm,ilm->i',cont_samples,self.J,cont_samples)/2
		...
		return (val_energies,control_energies,random_energies)
