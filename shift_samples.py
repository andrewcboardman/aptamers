...
# Make new directory
if not os.path.isdir(f'../test_data/output_{args.infile}/{args.outfile}_shifted'):
	os.mkdir(f'../test_data/output_{args.infile}/{args.outfile}_shifted')
# Create an array to write shifted samples into
samples_shifted = np.memmap(f'../test_data/output_{args.infile}/{args.outfile}_shifted/samples.npy',mode='w+',dtype=int,shape=(n_samples,L,3))
# Copy metadata
np.savetxt(f'../test_data/output_{args.infile}/{args.outfile}_shifted/samples_inf.txt',(L,Nc,ns,nb,nw,b))
# Generate random shifts
shifts = np.random.randint(-5,6,size=n_samples)
print(shifts.size)
